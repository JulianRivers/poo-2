...
public class RegistroMotoController {
	...
	void ingresar(ActionEvent event) throws Exception {
		try {
			Fachada.getInstance().ingresarVehiculo(txtPlaca.getText(),
					cbMarca.getSelectionModel().getSelectedIndex() + 1, Integer.parseInt(cbCilindraje.getSelectionModel().getSelectedItem()),
					Integer.parseInt(cbModelo.getSelectionModel().getSelectedItem()), cbTipos.getSelectionModel().getSelectedItem(), 
	...
	void initialize() throws SQLException, RemoteException {
		...
		cbCilindraje.getItems()
				.addAll(Fachada.getInstance().listarCilindraje(DataTransfer.getInstance().getVehiculo()));
